...
interface LexerExports {
  ...
  clickhouse_lexer_create: (
    lexer: number, queryBegin: number, queryEnd: number, maxQuerySize: number,
  ) => void;
  clickhouse_lexer_next_token: (lexer: number, tokenBegin: number, tokenEnd: number) => number;
...
export function lexerIsReady(): boolean {
...
// error token).
export function tokenizeSync(query: string): Token[] {
  if (!lexerExports) {
  ...
  if (memoryOffset > exports.memory.buffer.byteLength) {
    exports.memory.grow(Math.ceil((memoryOffset - exports.memory.buffer.byteLength) / 65536));
  }
  ...
  for (;;) {
    const tokenType = exports.clickhouse_lexer_next_token(lexerOffset, tokenBegin, tokenEnd);
    if (exports.clickhouse_lexer_token_is_error(tokenType)
    ...
    if (end <= previousEnd) {
      throw new Error(`SQL lexer stopped advancing at offset ${end - queryBegin} of ${bytes.length}`);
    }
    previousEnd = end;
